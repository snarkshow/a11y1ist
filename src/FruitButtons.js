import React, { Component } from 'react';
import firebase from './firebase';

class FruitButtons extends Component {
    constructor() {
        super();
        this.state = {
            fruit: [],
        }
    }
    handleClick = (event) => {
        //identify the button that was clicked
        //locate text value in the button
        //push to firebase
        const buttonValue = event.target.textContent;
        const dbRef = firebase.database().ref(`savedItems`);
        dbRef.push(buttonValue);
        // const users = firebase.database().ref('users');
        console.log(`I've been clicked`, buttonValue)
    }

    render() {
        return (
            <div className="fruitList">

                {this.state.fruit.map((fruit, index) => {
                    return <button className="groceryButton" key={index} onClick={this.handleClick}>{fruit}</button>
                })}

            </div>
        );
    }

    componentDidMount() {
        const dbRef = firebase.database().ref("allFruit");
        dbRef.on('value', (response) => {
            const newState = [];
            const data = response.val();
            for (let key in data) {
                newState.push(data[key]);
            }
            this.setState({
                fruit: newState
            });
            // console.log(response.val());
        });
    }
}
export default FruitButtons;